...
void SendControllerStatus() {
  ...
  if (invertX) {
    if (xRead > (xCenter - xDeadzone) && xRead < (xCenter + xDeadzone)) // deadzone
      ControllerPacket.Controller.JoyX = 0x80;
    else if (xRead < xCenter)
      ControllerPacket.Controller.JoyX = map(xRead, xMin + xAntiDeadzone, xCenter - xDeadzone, 0xFF, 0x81);
    else if (xRead > xCenter)
      ControllerPacket.Controller.JoyX = map(xRead, xCenter + xDeadzone, xMax - xAntiDeadzone, 0x7F, 0x00);
  } else {
    if (xRead > (xCenter - xDeadzone) && xRead < (xCenter + xDeadzone)) // deadzone
      ControllerPacket.Controller.JoyX = 0x80;
    else if (xRead < xCenter)
      ControllerPacket.Controller.JoyX = map(xRead, xMin + xAntiDeadzone, xCenter - xDeadzone, 0x00, 0x7F);
    else if (xRead > xCenter)
      ControllerPacket.Controller.JoyX = map(xRead, xCenter + xDeadzone, xMax - xAntiDeadzone, 0x81, 0xFF);
  }

  adc_select_input(1);
  uint8_t yRead = adc_read() >> 4;
  if (invertY) {
    if (yRead > (yCenter - yDeadzone) && yRead < (yCenter + yDeadzone)) // deadzone
      ControllerPacket.Controller.JoyY = 0x80;
    else if (yRead < yCenter)
      ControllerPacket.Controller.JoyY = map(yRead, yMin + yAntiDeadzone, yCenter - yDeadzone, 0xFF, 0x81);
    else if (yRead > yCenter)
      ControllerPacket.Controller.JoyY = map(yRead, yCenter + yDeadzone, yMax - yAntiDeadzone, 0x7F, 0x00);
  } else {
    if (yRead > (yCenter - yDeadzone) && yRead < (yCenter + yDeadzone)) // deadzone
      ControllerPacket.Controller.JoyY = 0x80;
    else if (yRead < yCenter)
      ControllerPacket.Controller.JoyY = map(yRead, yMin + yAntiDeadzone, yCenter - yDeadzone, 0x00, 0x7F);
    else if (yRead > yCenter)
...
void SendBlockReadResponsePacket(uint func) {
  ...
  if (func == FUNC_VIBRATION) { // PuruPuru AST Block Read
    PuruPuruDataPacket.PuruPuruBlockRead.Address = 0;
  ...
  } else if (func == FUNC_MEMORY_CARD) { // Memory Card Block Read
...
if (triggerMode) { // analog
  ...
  if (oledType) { // set up SPI for SSD1331 OLED
